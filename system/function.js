...
if(delSessi){
setInterval(() => {
let size = alice.getDirSize(`./${global.session}`)
if(size > 5000000){
...
await sleep(2000)
alice.sendMessage(i,{contextInfo,text})
}
}
}
} else {
...
waktu: waktu
  ...
  for (let i of groupTime ){
    ...
    alice.groupSettingUpdate(chat.id, 'not_announcement')
    chat.open = 0
    groupTime.splice(groupTime.indexOf(i,1))
    let text = `*Tepat waktu* grup dibuka oleh admin\n sekarang member dapat mengirim pesan`
    alice.sendMessage(chat.id,{text})
    ...
    alice.groupSettingUpdate(chat.id, 'announcement')
    chat.close = 0
...
setInterval(() => {
try{
var size = alice.getDirSize(`./.cache`)
} catch{
var size = 0
}
if(size > 5000000){
